...
{
  clause *res = 0;
  unsigneds trail = solver->trail;
  size_t size_stack = SIZE_STACK(trail);
  unsigned propagated = solver->propagated;
  while (!res && propagated < size_stack)
  { 
    
    const unsigned lit = trail.begin[propagated];
    propagated++;
    if(propagated < size_stack) __builtin_prefetch(BEGIN_WATCHES(WATCHES(NOT(trail.begin[propagated]))),0,0); // prefetching q for next function call
