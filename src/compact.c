...
compact_literal(kissat *solver, unsigned dst_lit, unsigned src_lit)
{
  ...
  // unsigned dst_qidx = solver->links[dst_idx].q_idx;

  // if(!DISCONNECTED(dst_qidx))   // else the value will be updated when its enqueued
  //   solver->exp_queue[dst_qidx].value = solver->values[dst_lit];
